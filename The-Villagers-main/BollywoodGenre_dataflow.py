...
from apache_beam.io.gcp.bigquery import ReadFromBigQuery, WriteToBigQuery
    ...
    # Implementing the DoFn on format class, splitting the genre column at commas. 
class FormatGenre(beam.DoFn):
  def process(self, element):
    ...
    #If genre is not null/none, then split the multiple genres by a comma to place into a record
    if genre != None:
        ...
        if len(split_name) > 1:
            for i in range (0, len(split_name)-1): 
                name = split_name[i] 
                record = {'titleId': titleId, 'genre': name}
                yield record
    #Else, if null/None, add to record
    else:
        record = {'titleId': titleId, 'genre': genre}
        return [record]
...
def run():
    ...
    #runner is dataflow runner instead of direct runner
     ...
     project=PROJECT_ID,
     job_name='bollywoodgenre', #globally unique
     temp_location=BUCKET + '/temp',
     region='us-central1')

     p = beam.pipeline.Pipeline(options=options)
    
         # selecting the titleID, genre from datamart Bollywood
     sql = 'SELECT titleId, Genre FROM datamart.Bollywood' #no limit clause, this will process all of out data in a distributive manner
     ...
     dataset_id = 'datamart'
     table_id = PROJECT_ID + ':' + dataset_id + '.' + 'bollywoodGenre_Dataflow'
     schema_id = 'titleId:STRING,genre:STRING'
