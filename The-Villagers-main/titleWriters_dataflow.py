...
def run():
    ...
    #runner is dataflow runner instead of direct runner
     ...
     project=PROJECT_ID,
     job_name='titlewriters', #globally unique
     temp_location=BUCKET + '/temp',
     region='us-central1')

     p = beam.pipeline.Pipeline(options=options)
    
         # selecting the titleId, writers from datamart title crew 
     sql = 'SELECT titleId, writers FROM datamart.Title_crew' #no limit clause, this will process all of out data in a distributive manner
     ...
     dataset_id = 'datamart'
     table_id = PROJECT_ID + ':' + dataset_id + '.' + 'titleWriters_Dataflow'
