  ...
  def process(self, element):
    ...
    if len(split_name) > 1:
        for i in range (0, len(split_name)-1): 
            name = split_name[i] 
            record = {'titleId': titleId, 'genres': name}
            yield record
    else:
        split_name = cast.split(' ')
        name = split_name[0]
...
def run():
    ...
    #runner is dataflow runner instead of direct runner
     ...
     project=PROJECT_ID,
     job_name='titlegenres', #globally unique
     temp_location=BUCKET + '/temp',
     region='us-central1')

     p = beam.pipeline.Pipeline(options=options)
    
         # selecting the titleId, genres from datamart title basics
     sql = 'SELECT titleId, genres FROM datamart.Title_basics' #no limit clause, this will process all of out data in a distributive manner
     ...
     dataset_id = 'datamart'
     table_id = PROJECT_ID + ':' + dataset_id + '.' + 'titleGenres_Dataflow'
     schema_id = 'titleId:STRING,genres:STRING'
